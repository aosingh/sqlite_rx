...
def zap_client():
    with get_server_auth_files() as auth_files:
        curve_dir, server_key_id, server_public_key, server_private_key = (
            auth_files
        )
        ...
        shutil.copyfile(
            client_public_key,
            os.path.join(
                curve_dir, "authorized_clients", "{}.key".format(client_key_id)
            ),
        )

        auth_config = {sqlite3.SQLITE_OK: {sqlite3.SQLITE_DROP_TABLE}}
        server = SQLiteServer(
            bind_address="tcp://127.0.0.1:5001",
            use_zap_auth=True,
            use_encryption=True,
            curve_dir=curve_dir,
        ...
        client = SQLiteClient(
            connect_address="tcp://127.0.0.1:5001",
            server_curve_id=server_key_id,
