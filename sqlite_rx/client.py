...
class SQLiteClient(threading.local):
    ...
    def _init_client(self):
        ...
        if self._encrypt:
            if not self.server_curve_id:
                raise MissingServerCurveKeyID("Please provide the name of the server key_id to be used for Curve")
    ...
    def execute(self, query, *args, **kwargs):
        ...
        if execute_script and execute_many:
            raise InvalidRequest("Both `execute_script` and `execute_many` cannot be True")
