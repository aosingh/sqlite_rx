...
from sqlite_rx import __version__, get_default_logger_settings
...
def print_help():
    ...
    console.print(
        f"[bold]sqlite-server[/bold]   :paw_prints:", justify="center"
    )
    console.print()
    console.print(
    ...
    console.print()
    console.print(
        "Usage: [bold]sqlite-server[/bold] [cyan][OPTIONS][/cyan] ",
        justify="left",
    )
    console.print()
    ...
    table.add_row(
        "--curvezmq/--no-curvezmq",
        "Enable/Disable CurveZMQ\n" "Default value is [bold][cyan]False",
    )
    table.add_row(
        "-c --curve-dir [cyan]PATH",
        "Path to the Curve key directory\n"
        "Default value is [bold][cyan]~/.curve",
    )
    table.add_row("-k --key-id [cyan]CURVE KEY ID", "Server's Curve Key ID")
    table.add_row(
        "-b --backup-database [cyan]PATH", "Path to the backup database"
    )
    table.add_row(
        "-i --backup-interval [cyan]FLOAT", "Backup interval in seconds"
    )
...
@click.option(
    ...
    help="Logging level",
    type=click.Choice(
        "CRITICAL FATAL ERROR WARN WARNING INFO DEBUG NOTSET".split()
    ),
    show_default=True,
)
@click.option(
...
@click.option(
    "--key-id", "-k", type=click.STRING, help="Server key ID", default=None
)
@click.option(
    "--backup-database",
    "-b",
