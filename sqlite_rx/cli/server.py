...
@click.option('--log-level', default='INFO', help="Logging level",
              type=click.Choice(list(logging._nameToLevel.keys())), show_default=True)
@click.option('--advertise-host', default='0.0.0.0',
              help='Host address on which to run the SQLiteServer',
              show_default=True)
@click.option('--port', default='5000', help="Port on which SQLiteServer will listen for connection requests",
              type=str, show_default=True)
@click.option('--database', default=':memory:',
              help='Path like object giving the database name. You can use `:memory:` for an in-memory database',
              show_default=True)
@click.option('--zap/--no-zap', help='True, if you want to enable ZAP authentication',
              default=False, show_default=True)
@click.option('--curvezmq/--no-curvezmq', help='True, if you want to enable CurveZMQ encryption',
              default=False, show_default=True)
