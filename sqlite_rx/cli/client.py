...
def print_help():
    ...
    console.print()
    console.print(
        "A simple, fast and secure client for the SQLite database.",
        justify="center",
    )
    console.print()
    console.print(
        "Usage: [bold]sqlite-client [OPTIONS] exec[/bold] [cyan]<query> [/cyan] ",
        justify="left",
    )
    console.print()
    ...
    table.add_row(
        "-l, --log-level [bold][cyan]LOG_LEVEL",
        "CRITICAL FATAL ERROR WARN WARNING INFO DEBUG NOTSET\n"
        "Default level is [bold][cyan]CRITICAL",
    )
    table.add_row(
        "-a, --connect-address [bold][cyan]tcp://<host>:<port>",
    ...
    table.add_row(
        "-d --curve-dir [cyan]PATH",
        "Path to the Curve key directory\n"
        "Default value is [italic][bold][cyan]~/.curve",
    )
    table.add_row(
        "-c --client-key-id [cyan]CURVE KEY ID", "Client's Curve Key ID"
    )
...
@click.option("--curve-dir", "-d", help="Curve Key directory", default=None)
@click.option(
    "--client-key-id",
    "-c",
    type=click.STRING,
